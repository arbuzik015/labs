...
class WorkWindow(tk.Toplevel):
    COLUMNS = ["Дата", "Место", "Температура (°C)"]
    ...
    def refresh(self):
        ...
        for item in self.data:
            self.tree.insert("", tk.END, values=(item.date, item.location, f"{item.value:.1f}"))
    ...
    def add(self, date, location, value):
        self.data.append(Temperature(date, location, value))
        self.refresh()
...
class AddDialog:
    def __init__(self, parent):
        ...
        labels = ["Дата (ДД.ММ.ГГГГ):", "Место:", "Температура:"]
        self.entries = []
    ...
    def ok(self):
        ...
        self.parent.add(vals[0], vals[1], temperature)
        self.win.destroy()
